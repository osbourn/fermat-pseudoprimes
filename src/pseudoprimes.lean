...
private theorem next_in_sequence (n : ℕ) (n_psp : pseudoprime n 2) : pseudoprime (2^n - 1) 2 :=
begin
  generalize h : 2^n - 1 = m,
  have m_gt_one : m > 1 := sorry,
  have n_gt_one : n > 1 := sorry,
  have n_gt_zero : n > 0 := sorry,
  have m_coprime_two : nat.coprime m 2 := sorry,
  have m_not_prime : ¬nat.prime m := begin
    rw ← h,
    have n_coprime_two := and.elim_left n_psp,
  ...
  have n_dvd : n ∣ (m - 1) := begin
    rw ← h,
    have h₁ : 2 ∣ (2^n) := begin
      refine dvd_pow_self 2 _,
      exact ne_of_gt n_gt_zero
    end,
    have h₂ : (2^(n - 1)) * 2 = (2^n) := sorry,
    have h₃ : (2^(n - 1) - 1) * 2 = (2^n - 1 * 2) := begin
      rw ← h₂,
      sorry,
    end,
    sorry
  end,
  generalize h₁ : (m - 1) / n = k,
  have k_mul_n : k * n = m - 1 := begin
    rw ← h₁,
    exact nat.div_mul_cancel n_dvd
  end,
  have m_div_two_pow_kn : m ∣ (2^(k*n) - 1) := begin
    rw ← h,
    apply dvd_lem,
    norm_num,
    norm_num,
